...
export default function SalesBillviewController($scope, $routeParams) {
    const invoicerefno = $routeParams.salrfno;
    ...
    $scope.billinfoforview = {};
    initLoadDatas();
    const GetUnClosedBill = async () => {
        if ($scope.isLoading) return;
        $scope.isLoading = true;
        const res = await apis.GET(`sales/get.php?billno=${invoicerefno}`);
        if (!res.isSuccess) {
            alert(res.msg);
            $scope.isLoading = false;
